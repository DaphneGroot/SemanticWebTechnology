...
TUPLE_AMOUNT = 2 #how many tuples?
if TUPLE_AMOUNT > 1:
	TUPLE_FILE = str(TUPLE_AMOUNT)+'triples'
else:
	TUPLE_FILE = str(TUPLE_AMOUNT)+'triple'
...
def testData(predicateDict, category):
	...
	score = 0
	treeTest = ET.parse('WebNLG/dev/'+str(TUPLE_AMOUNT)+'triples/'+TUPLE_FILE+'_allSolutions_'+category+'_dev_challenge.xml')
...
def run(category):
	tree = ET.parse('WebNLG/train/'+str(TUPLE_AMOUNT)+'triples/'+TUPLE_FILE+'_allSolutions_AllExceptCC_train_challenge.xml')
	root = tree.getroot()
	predicateDict = {}
...
if __name__ == '__main__':
	main(categorySetting = 'one')	#one or all